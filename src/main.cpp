...
#if defined(NDEBUG)
# error "Gaoshcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xba508d10cb25301d1dcdec26d7738f02895d1ed6593073775c9f1928f333ff49");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gaoshcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gaoshcoin Signed Message:\n";
...
{
    // Gaoshcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 1951 * COIN;

    // Subsidy is cut in half every 8400000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 8400000); // Gaoshcoin: 8400k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Gaoshcoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Gaoshcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gaoshcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gaoshcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x54b2f52f2b1f4ffca5de818165bb2eef8a63b2bace4ad831a1485a177e812595");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Ding Jiaoshu";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1951 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045b2face1f67f9345be0c89e23eafadcc7d836105f9fb126fbb9098a27fcdb285158b0c4173c0c9bf23b703228617ae0bb1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1568116169;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084849483;

        if (fTestNet)
        {
            block.nTime    = 1568115996;
            block.nNonce   = 387945997;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb2988d0815d50fe7fa9237da0d07bcfd079c6d15b93cb9d538c16208a26f5467"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc0, 0xb2, 0xd3 }; // Gaoshcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GaoshcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GaoshcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GaoshcoinMiner(CWallet *pwallet)
{
    printf("GaoshcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gaoshcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GaoshcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GaoshcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GaoshcoinMiner, pwallet));
}
